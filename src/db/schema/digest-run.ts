...
export const digestRuns = sqliteTable("digest_runs",
{
  ...
  modelUsage: text("model_usage", { mode: "json" }).$type<DigestModelUsageSnapshot>(),
},
(table) => [
  uniqueIndex("digest_runs_user_running_unique")
    .on(table.userId)
    .where(sql`${table.status} = 'running'`),
  index("digest_runs_user_started_idx").on(table.userId, table.startedAt),
...
export const digestRunFeeds = sqliteTable("digest_run_feeds",
{
  id: text("id").primaryKey().$defaultFn(() => crypto.randomUUID()),
  runId: text("run_id")
    .notNull()
