...
export interface UpsertDigestStoryInput { content: DigestStoryContent; profileVersion: number; summaryVersion: string; generatedAt?: number }
export interface StoredDigestStory extends DigestStoryContent { id: string; digestId: string; profileVersion: number; summaryVersion: string; generatedAt: number }
export interface ListRecentStoryReferencesOptions { limit?: number; since?: number }
export interface ReusableDigestStoryLookupInput { storyId: string; storyVersion: number; profileVersion: number; summaryVersion: string }
export interface ReusableDigestStoryPoints extends ReusableDigestStoryLookupInput { points: DigestStoryContent["points"] }

export function listRecentStoryReferences(database: Database, userId: string, options: ListRecentStoryReferencesOptions = {}): StoryReference[] { const validUserId = z.string().uuid().parse(userId);
...
}).from(stories).where(and(...conditions)).orderBy(desc(stories.lastUpdatedAt), asc(stories.id)).limit(parsed.limit).all();
...
export function findItemAnalysis(database: Database, itemId: string, fingerprint: string, analyzerVersion: string): StoredItemAnalysis | null { const rows = database.select().from(itemAnalyses).where(and(eq(itemAnalyses.itemId, z.string().uuid().parse(itemId)), eq(itemAnalyses.fingerprint, fingerprint), eq(itemAnalyses.analyzerVersion, analyzerVersion))).limit(1).all();
return rows[0] ? parseAnalysis(rows[0]) : null; }
export interface ItemAnalysisLookupInput { itemId: string; fingerprint: string }

export function listItemAnalyses(database: Database, inputs: ItemAnalysisLookupInput[], analyzerVersion: string): StoredItemAnalysis[] { const parsedInputs = z.array(z.object({ itemId: z.string().uuid(), fingerprint: z.string().min(1) }).strict()).parse(inputs);
const validAnalyzerVersion = z.string().min(1).parse(analyzerVersion);
if (parsedInputs.length === 0) return [];
const rows = database.select().from(itemAnalyses).where(and(
...
export function upsertItemAnalysis(database: Database, input: UpsertItemAnalysisInput): StoredItemAnalysis { const parsed = z.object({ itemId: z.string().uuid(), fingerprint: z.string().min(1), analyzerVersion: z.string().min(1), analysis: analysisSchema, analyzedAt: z.number().int().nonnegative().default(() => Date.now()) }).parse(input);
const rows = database.insert(itemAnalyses).values(parsed).onConflictDoUpdate({ target: itemAnalyses.itemId, set: { fingerprint: parsed.fingerprint, analyzerVersion: parsed.analyzerVersion, analysis: parsed.analysis, analyzedAt: parsed.analyzedAt } }).returning().all();
return parseAnalysis(rows[0]); }
export function upsertItemAnalyses(database: Database, inputs: UpsertItemAnalysisInput[]): StoredItemAnalysis[] { const parsed = z.array(z.object({ itemId: z.string().uuid(), fingerprint: z.string().min(1), analyzerVersion: z.string().min(1), analysis: analysisSchema, analyzedAt: z.number().int().nonnegative().default(() => Date.now()) }).strict()).superRefine((values, context) => {
...
const rows = database.insert(itemAnalyses).values(parsed).onConflictDoUpdate({
...
export function upsertResolvedStories(database: Database, userId: string, inputs: ResolvedStoryCandidate[], now = Date.now()): PersistedStoryCandidate[] { const candidates = z.array(candidateSchema).superRefine((values, context) => {
...
return database.transaction((tx) => {
  ...
  if (candidateItemIds.length) {
    const owned = db.select({ id: items.id }).from(items).innerJoin(feeds, eq(items.feedId, feeds.id)).innerJoin(sources, eq(feeds.sourceId, sources.id)).where(and(inArray(items.id, candidateItemIds), eq(sources.userId, validUserId))).all();
    if (owned.length !== candidateItemIds.length) throw new Error("story items must all be owned by the story user");
  }

  const existingStories = db.select().from(stories).where(and(eq(stories.userId, validUserId), inArray(stories.canonicalKey, candidates.map((candidate) => candidate.canonicalKey)))).all();
  ...
  if (missingCandidates.length) {
    const inserted = db.insert(stories).values(missingCandidates.map((candidate) => ({ userId: validUserId, canonicalKey: candidate.canonicalKey, title: candidate.title, topics: candidate.topics, entities: candidate.entities, firstSeenAt: now, lastUpdatedAt: now }))).returning().all();
  ...
  const storedDevelopments = db.select().from(storyDevelopments).where(inArray(storyDevelopments.storyId, storyIds)).all();
  ...
  if (missingDevelopments.length) {
    const inserted = db.insert(storyDevelopments).values(missingDevelopments.map(({ story, development }) => ({ storyId: story.id, canonicalKey: development.canonicalKey, type: development.type, title: development.title, occurredAt: development.occurredAt, firstSeenAt: now, lastUpdatedAt: now }))).returning().all();
  ...
  for (const candidate of candidates) {
    ...
    if (storyMetadataChanged) db.update(stories).set({ title: candidate.title, topics: candidate.topics, entities: candidate.entities, lastUpdatedAt: now }).where(eq(stories.id, story.id)).run();
    for (const development of candidate.developments) {
      ...
      if (!newDevelopmentIds.has(stored.id) && (stored.type !== development.type || stored.title !== development.title || stored.occurredAt !== occurredAt)) {
        db.update(storyDevelopments).set({ type: development.type, title: development.title, occurredAt, lastUpdatedAt: now }).where(eq(storyDevelopments.id, stored.id)).run();
  ...
  const currentMemberships = candidateItemIds.length
    ? db.select({ developmentId: storyItems.developmentId, itemId: storyItems.itemId, fingerprint: storyItems.itemFingerprint, storyId: storyDevelopments.storyId }).from(storyItems).innerJoin(storyDevelopments, eq(storyItems.developmentId, storyDevelopments.id)).where(inArray(storyItems.itemId, candidateItemIds)).all()
    : [];
  ...
  if (replacementItemIds.length) {
    db.delete(storyItems).where(inArray(storyItems.itemId, replacementItemIds)).run();
    db.insert(storyItems).values(replacementRows).run();
  }
  if (changedDevelopmentIds.size) db.update(storyDevelopments).set({ version: sql`${storyDevelopments.version} + 1`, lastUpdatedAt: now }).where(inArray(storyDevelopments.id, [...changedDevelopmentIds])).run();
  if (changedStoryIds.size) db.update(stories).set({ version: sql`${stories.version} + 1`, lastUpdatedAt: now }).where(inArray(stories.id, [...changedStoryIds])).run();
  const finalStories = db.select().from(stories).where(inArray(stories.id, storyIds)).all();
...
export function upsertResolvedStory(database: Database, userId: string, input: ResolvedStoryCandidate, now = Date.now()): PersistedStoryCandidate { return (upsertResolvedStories(database, userId, [input], now))[0]; }

export function listStoryMembers(database: Database, userId: string, storyId: string): StoryMember[] { const rows = database.select({ developmentId: storyDevelopments.id, developmentCanonicalKey: storyDevelopments.canonicalKey, developmentVersion: storyDevelopments.version, itemFingerprint: storyItems.itemFingerprint, item: items }).from(stories).innerJoin(storyDevelopments, eq(stories.id, storyDevelopments.storyId)).innerJoin(storyItems, eq(storyDevelopments.id, storyItems.developmentId)).innerJoin(items, eq(storyItems.itemId, items.id)).where(and(eq(stories.id, z.string().uuid().parse(storyId)), eq(stories.userId, z.string().uuid().parse(userId)))).orderBy(asc(storyDevelopments.occurredAt), asc(items.date)).all();
...
export function replaceDigestStories(database: Database,
userId: string,
digestId: string,
inputs: UpsertDigestStoryInput[],): StoredDigestStory[] { const validUserId = z.string().uuid().parse(userId);
...
const parsed = inputs.map((input) => ({
  ...
  summaryVersion: z.string().min(1).parse(input.summaryVersion),
...
return database.transaction((tx) => {
  ...
  if (parsed.length) {
    ...
    const ownedStories = db
      .select({ id: stories.id })
      .from(stories)
      .where(and(eq(stories.userId, validUserId), inArray(stories.id, storyIds))).all();
    if (ownedStories.length !== new Set(storyIds).size) {
      throw new Error("digest stories must be owned by user");
    }
    db.delete(digestStories).where(and(
    ...
    db
      ...
      .values(parsed.map(({ content, profileVersion, summaryVersion, generatedAt }) => ({
      ...
      .onConflictDoUpdate({
        ...
        set: {
          ...
          summaryVersion: sql`excluded.summary_version`,
    ...
    db.delete(digestStories).where(eq(digestStories.digestId, validDigestId)).run();
  }

  db
    .update(digests)
    .set({ contentMode: "stories" })
    .where(eq(digests.id, validDigestId)).run();
  return listDigestStories(db, validUserId, validDigestId);
}, { behavior: "immediate" }); }

export function upsertDigestStory(database: Database, userId: string, digestId: string, input: UpsertDigestStoryInput): StoredDigestStory { const validUserId = z.string().uuid().parse(userId); const validDigestId = z.string().uuid().parse(digestId);
const content = digestContentSchema.parse(input.content); const profileVersion = z.number().int().positive().parse(input.profileVersion); const summaryVersion = z.string().min(1).parse(input.summaryVersion); const generatedAt = z.number().int().nonnegative().parse(input.generatedAt ?? Date.now());
return database.transaction((tx) => {
  const db = tx as Database;
  const ownership = db.select({ digestId: digests.id, storyId: stories.id }).from(digests).innerJoin(stories, and(eq(stories.id, content.storyId), eq(stories.userId, digests.userId))).where(and(eq(digests.id, validDigestId), eq(digests.userId, validUserId))).limit(1).get();
  if (!ownership) throw new Error("digest and story must be owned by user");
  db.update(digests).set({ contentMode: "stories" }).where(eq(digests.id, validDigestId)).run();
  db.insert(digestStories).values({ digestId: validDigestId, storyId: content.storyId, storyVersion: content.storyVersion, profileVersion, summaryVersion, title: content.title, topics: content.topics, entities: content.entities, points: content.points, sources: content.sources, relevanceScore: content.relevanceScore, matchedInterestRuleIds: content.matchedInterestRuleIds, generatedAt }).onConflictDoUpdate({
    target: [digestStories.digestId, digestStories.storyId],
    set: { storyVersion: content.storyVersion, profileVersion, summaryVersion, title: content.title, topics: content.topics, entities: content.entities, points: content.points, sources: content.sources, relevanceScore: content.relevanceScore, matchedInterestRuleIds: content.matchedInterestRuleIds, generatedAt },
  }).run();
  const stored = listDigestStories(db, validUserId, validDigestId);
  return stored.find((story) => story.storyId === content.storyId)!;
}, { behavior: "immediate" }); }

export function listDigestStories(database: Database, userId: string, digestId: string): StoredDigestStory[] { const rows = database.select({ row: digestStories }).from(digestStories).innerJoin(digests, eq(digestStories.digestId, digests.id)).where(and(eq(digestStories.digestId, z.string().uuid().parse(digestId)), eq(digests.userId, z.string().uuid().parse(userId)))).orderBy(asc(digestStories.generatedAt), asc(digestStories.id)).all();
return rows.map(({ row }) => ({ id: row.id, digestId: row.digestId, profileVersion: row.profileVersion, summaryVersion: z.string().min(1).parse(row.summaryVersion), generatedAt: row.generatedAt, ...digestContentSchema.parse({ storyId: row.storyId, storyVersion: row.storyVersion, title: row.title, topics: row.topics, entities: row.entities, points: row.points, sources: row.sources, relevanceScore: row.relevanceScore, matchedInterestRuleIds: row.matchedInterestRuleIds }) })); }

export function listReusableDigestStoryPoints(database: Database,
userId: string,
inputs: ReusableDigestStoryLookupInput[],
excludeDigestId?: string,): ReusableDigestStoryPoints[] { const validUserId = z.string().uuid().parse(userId);
const parsedInputs = z.array(z.object({
  storyId: z.string().uuid(),
  storyVersion: z.number().int().positive(),
  profileVersion: z.number().int().positive(),
  summaryVersion: z.string().min(1),
}).strict()).parse(inputs);
if (parsedInputs.length === 0) return [];

const conditions = [
  eq(digests.userId, validUserId),
  eq(digests.status, "complete"),
  or(...parsedInputs.map((input) => and(
    eq(digestStories.storyId, input.storyId),
    eq(digestStories.storyVersion, input.storyVersion),
    eq(digestStories.profileVersion, input.profileVersion),
    eq(digestStories.summaryVersion, input.summaryVersion),
  ))),
];
if (excludeDigestId !== undefined) {
  conditions.push(ne(digests.id, z.string().uuid().parse(excludeDigestId)));
}
const ranked = database.select({
  storyId: digestStories.storyId,
  storyVersion: digestStories.storyVersion,
  profileVersion: digestStories.profileVersion,
  summaryVersion: digestStories.summaryVersion,
  points: digestStories.points,
  rank: sql<number>`row_number() over (partition by ${digestStories.storyId}, ${digestStories.storyVersion}, ${digestStories.profileVersion}, ${digestStories.summaryVersion} order by ${digestStories.generatedAt} desc, ${digestStories.id} desc)`.as("rank"),
})
  .from(digestStories)
  .innerJoin(digests, eq(digestStories.digestId, digests.id))
  .where(and(...conditions))
  .as("ranked_digest_stories");
const rows = database.select({
  storyId: ranked.storyId,
  storyVersion: ranked.storyVersion,
  profileVersion: ranked.profileVersion,
  summaryVersion: ranked.summaryVersion,
  points: ranked.points,
}).from(ranked).where(eq(ranked.rank, 1)).all();
const parsedRows = rows.map((row) => ({
  storyId: row.storyId,
  storyVersion: row.storyVersion,
  profileVersion: row.profileVersion,
  summaryVersion: z.string().min(1).parse(row.summaryVersion),
  points: z.array(summaryPointSchema).parse(row.points),
}));
const rowByKey = new Map(parsedRows.map((row) => [
  `${row.storyId}:${row.storyVersion}:${row.profileVersion}:${row.summaryVersion}`,
  row,
]));
return parsedInputs.flatMap((input) => {
  const row = rowByKey.get(`${input.storyId}:${input.storyVersion}:${input.profileVersion}:${input.summaryVersion}`);
  return row ? [row] : [];
}); }

export function findLatestDeliveredStoryVersions(database: Database, userId: string, storyIds: string[], excludeDigestId?: string): Map<string, number> { const validUserId = z.string().uuid().parse(userId);
...
const rows = database.select({ storyId: digestStories.storyId, version: max(digestStories.storyVersion) })
  .from(digestStories)
  .innerJoin(digests, eq(digestStories.digestId, digests.id))
  .where(and(...conditions))
  .groupBy(digestStories.storyId).all();
return new Map(rows.filter((row) => row.version !== null).map((row): [string, number] => [row.storyId, row.version!])); }

export function findLatestDeliveredStoryVersion(database: Database, userId: string, storyId: string, excludeDigestId?: string): number | null { return (findLatestDeliveredStoryVersions(database, userId, [storyId], excludeDigestId)).get(storyId) ?? null; }
