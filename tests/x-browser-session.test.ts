...
import {
  launchChromeProcess,
  type XChromeProcess,
} from "../src/connectors/x/chrome-process.ts";
...
test("a failed unmanaged close keeps the profile lease and a later close releases it", async () => {
  const root = await mkdtemp(join(tmpdir(), "morning-post-x-close-retry-"));
  ...
  try {
    const session = await sessions.openUnmanaged(PROFILE_ID, "home");

    await assertRejects(() => session.close(), Error, "terminate failed");
    await assertRejects(
      () => sessions.removeProfile(PROFILE_ID),
      Error,
      "Timed out waiting for the X browser profile",
    );

    await session.close();
    await sessions.removeProfile(PROFILE_ID);
    assertStrictEquals(terminateCalls, 2);
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});

test("an aborted unmanaged launch releases its lease after the process later exits", async () => {
  const root = await mkdtemp(join(tmpdir(), "morning-post-x-aborted-launch-"));
  ...
  try {
    await assertRejects(
      () => sessions.openUnmanaged(PROFILE_ID, "home", controller.signal),
      Error,
      "launch cancelled",
    );
    running = false;
...
test("launchChromeProcess terminates a spawned child and confirms its exit", async () => {
  const process = await launchChromeProcess("/bin/sleep", ["30"]);
  assertStrictEquals(process.running, true);
  await process.terminate();
  assertStrictEquals(process.running, false);
  await process.exited;
}, 15_000);

test("launchChromeProcess escalates to SIGKILL when SIGTERM is ignored", async () => {
  const process = await launchChromeProcess("/bin/sh", [
    "-c",
...
test("launchChromeProcess surfaces spawn failures without unhandled errors", async () => {
  await assertRejects(
    () => launchChromeProcess("/definitely/not/here/missing-binary", []),
    Error,
    "ENOENT",
  );
});
