...
function statusDescription(status: XLoginStatus | undefined): string {
  switch (status) {
    case "awaiting_login":
      return "Sign in to X and complete any 2FA in the dedicated browser using Morning Post's dedicated profile, not your daily browser profile. When finished, fully close the dedicated login browser — Verify reopens the same profile to check your login. With the default installed-Chrome setup, use Cmd-Q on macOS to fully quit; closing a tab or window is not enough there.";
    case "awaiting_chat_unlock":
      return "The dedicated browser is open at X Messages in Morning Post's dedicated profile. Unlock Chat, then fully close the dedicated login browser — Verify reopens the same profile to check your login. With the default installed-Chrome setup, use Cmd-Q on macOS to fully quit; closing a tab or window is not enough there.";
    case "complete":
      return "X is connected. Scheduled captures will run headlessly from Morning Post's dedicated profile.";
    case "error":
      return "The browser login session could not be verified. Start another session after fixing the issue in Morning Post's dedicated profile.";
    case "expired":
      return "The login window expired. Start another session to try again.";
    default:
      return "Start a dedicated browser login session using Morning Post's dedicated profile, not your daily browser profile.";
  }
}
  ...
  return (
    <div class="card">
      ...
      <p class="hint x-connect-intro">
        X capture uses a dedicated browser with a separate profile owned by
        Morning Post, not your daily browser profile. The default setup uses
        installed stable Chrome as that dedicated browser. You never give
        Morning Post an X password, 2FA code, cookie, or session credential
        here.
      </p>
      ...
      <div class="x-connect-safety" role="note">
        ...
        <ul class="bullet-list">
          <li>
            Start the dedicated browser login from this page. Morning Post opens
            the configured browser with its separate profile on the same
            desktop where its service is running. That desktop must have a
            display; a remote or headless server cannot display this window for
            you. You can use this page in Safari or Firefox, but the login
            itself happens in the dedicated browser.
          </li>
          <li>
            Sign in and complete any 2FA manually in the dedicated browser
            profile. When finished, fully close the dedicated login browser —
            Verify reopens the same profile to check your login, so closing it
            early is safe. With the default installed-Chrome setup, use Cmd-Q
            on macOS to fully quit; closing a tab or window is not enough
            there. If Chat unlock is required, the dedicated browser stays open
            at X Messages with the next instructions.
          </li>
          <li>
            After verification, scheduled captures run headlessly from Morning
            Post's dedicated profile. Do not use your daily browser profile for
        ...
        >
          ...
          <div class="x-connect-actions">
            <Show when={!isTerminalStatus(activeStatus())}>
              ...
              >
                {operation() === "verify" ? "Verifying…" : "Verify login"}
              </button>
              <button
                type="button"
                class="danger"
            ...
            <Show when={isTerminalStatus(activeStatus())}>
              ...
              >
                {operation() === "start"
                  ...
                  : hasXSource()
                  ? "Reconnect X with Morning Post's dedicated browser profile"
                  : "Connect X with Morning Post's dedicated browser profile"}
              </button>
            </Show>
          </div>
          <Show when={error()}>
